...
trips/                     ← ⭐ 여행별 데이터(여기만 바꾸면 새 여행)
  tokyo-pjcs-2026/         예시 여행(도쿄) — 가장 완성된 실제 예시
    plan.json  locations.json  shops.json  shopping.json  docs/*.pdf
  osaka-demo/              샘플 두 번째 여행(오사카 2박3일·테마 주황) — "데이터만 바꾸면 새 여행" 데모
  _template/               빈 여행 양식(복사해서 시작)
```

> 💡 **데모 체험**: `config.js`의 `id`를 `'osaka-demo'`·`theme`를 `'#e8590c'`로 바꾸고 `modules`에서 `terms/cards/guide/shops/shopping`을 `false`로 하면 → **헤더색·일정·위치·메뉴가 통째로 오사카로** 바뀌어요(엔진은 안 건드림). 확인 후 다시 `tokyo-pjcs-2026`으로 되돌리면 됩니다.
