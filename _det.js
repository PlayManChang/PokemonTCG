...
const IDS=[47734,47735,47736,48348,46793,46803];
...
await p.setRequestInterception(true);p.on('request',r=>{const t=r.resourceType();(t==='image'||t==='font'||t==='media')?r.abort():r.continue();});
const out=[];
for(const id of IDS){
  ...
  const d=await p.evaluate((EN)=>{
    ...
    const tIcon=rb.querySelector('.TopInfo .icon');let type='';if(tIcon){const c=[...tIcon.classList].find(x=>EN[x]);type=EN[c]||'';}
    const abilities=[],attacks=[];let sec='',special='',ruleText='';
    for(const el of rb.children){
      ...
      if(el.tagName==='P'){const tx=el.innerText.trim();if(!tx)continue;
        ...
        else if(!sec)ruleText+=(ruleText?' ':'')+tx;
      }
    }
    return {name_ja:name,stage,hp,type,abilities,attacks,special,ruleText};
  },EN);
  d.id=String(id);out.push(d);process.stdout.write('.');
}
await b.close();fs.writeFileSync('_det.json',JSON.stringify(out,null,2));console.log('\nsaved',out.length);
})();
